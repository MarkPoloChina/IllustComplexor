      ...
      fs.readdir(path, async (err, files) => {
        ...
        for (let item of files) {
          ...
          if (
            pid &&
            page &&
            FilenameComparator.isLikeImage(item) &&
            !MetaDB.existMeta({ sid: pid, source: "pixiv", page: page })
          ) {
            if (title)
              list.push({
                sid: pid,
                ...
                page: page,
                title: title,
                basePath: path.charAt(0) == "/" ? `file:/${path}` : path,
                baseType: type,
                filename: item,
                timestamp: timestamp,
              });
        ...
        resolve({
          status: 200,
          message: `完成母本导入，共${list.length}个文件被录入，共${
            filesCnt - list.length
          }个文件被忽略`,
        });
      });
    });
  ...
  static loadFromFiles = (paths, type, progress) => {
    return new Promise((resolve, reject) => {
      ...
      paths.forEach(async (_path, index) => {
          ...
          if (
            ...
            if (title)
              MetaDB.pushMeta({
                ...
                title: title,
                basePath:
                  path.parse(_path).dir.charAt(0) == "/"
                    ? `file:/${path.parse(_path).dir}`
                    : path.parse(_path).dir,
                baseType: type,
                filename: item,
              });
            else
        ...
        resolve({
          status: 200,
          message: `完成母本导入，共${successCnt}个文件被录入，共${
            paths.length - successCnt
          }个文件被忽略`,
        });
      });
    });
      ...
      fs.readdir(path, async (err, files) => {
        ...
        for (let item of files) {
          ...
          if (!pid || !page || !FilenameComparator.isLikeImage(item))
            ...
            MetaDB.existCopy(
              { sid: pid, source: "pixiv", page: page },
              { copyPath: path }
            )
          ) {
            ignoredList.push({
        ...
        resolve({
          status: 200,
          message: `完成副本导入，共${successCnt}个文件被录入，共${
            filesCnt - successCnt
          }个文件被忽略`,
          data: ignoredList,
        });
...
export class UpdateMeta {
   ...
   * @param {Object} [progress] 进度对象
   */
  static updateMetaFromDirectoryWithGiven = (
    path,
    metaKey,
    metaValue,
    progress
      ...
      fs.readdir(path, async (err, files) => {
        ...
        for (let item of files) {
          ...
          if (!pid || !page || !FilenameComparator.isLikeImage(item))
            ...
            list.findIndex((item) => {
              return (
                item.sid == pid && item.source == "pixiv" && item.page == page
              );
            }) == -1
          )
            ignoredList.push({
        ...
        resolve({
          status: 200,
          message: `完成元数据更新，共${successCnt}个文件被更新，共${
            filesCnt - successCnt
          }个文件被忽略`,
          data: ignoredList,
        });
...
export class RemoteLoader {
  static getPixivLoaderDtoListFromDirectory = (path, info, progress) => {
    return new Promise((resolve) => {
      let list = [];
      let logs = [];
      fs.readdir(path, async (err, files) => {
        ...
        for (let item of files) {
          index++;
          let log = { filename: item, status: "ready", bid: index };
          let pid = FilenameComparator.getMatchedPixivId(item);
          let page = FilenameComparator.getMatchedPixivPage(item);
          let title = FilenameComparator.getPxderPixivTitle(item);
          if (pid !== null && page !== null) {
            const dto = { meta: { pid: pid, page: page }, ...info, bid: index };
            if (title) dto.meta.title = title;
            list.push(dto);
          } else log.status = "ignore";
          logs.push(log);
          progress.value = Math.round((index / files.length) * 1000) / 10;
          await sleep();
        }
        resolve({ dto: list, log: logs });
      });
    });
  };

  static getPixivLoaderDtoListFromFiles = async (paths, info, progress) => {
    let list = [];
    let logs = [];
    let index = 0;
    for (let _path of paths) {
      ...
      logs.push(log);
      progress.value = Math.round((index / paths.length) * 1000) / 10;
      await sleep();
    }
    return { log: logs, dto: list };
  };
}
